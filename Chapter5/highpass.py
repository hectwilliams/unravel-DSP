...
if __name__ == '__main__':
    ...
    shift_sinusoid = np.cos( (C_2PI  * x_trunc) * np.divide(C_FFT_N, SAMPLES_2_PER_CYCLE) *  np.divide(1, C_FFT_N) )
    ...
    ax5.set_title('shift_signal - 2 samples per cycle', fontsize=6)
    ax5.tick_params(axis='x', labelsize=6)
    ...
    handles = [ Line2D([0], [0], label='31_tap highpass freq_resp ', color='orange') , Line2D([0], [0], label='31_tap freq_resp', color='black') ] 
